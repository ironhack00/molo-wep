...
import { allMarketingCategories } from "@/data/marketing/pricingData";

/**
 * Organism: MarketingPricingSection
 * Sección de pricing para marketing con múltiples categorías
 * Muestra solo servicios de marketing: Redes Sociales, Publicidad Pagada, SEO
 */
export function MarketingPricingSection() {
  ...
  // allMarketingCategories ya contiene solo las categorías de marketing
  // No necesita filtrado adicional
  const currentCategory = allMarketingCategories.find(cat => cat.id === activeCategory);
  ...
  return (
    <section 
      className={cn("relative overflow-x-hidden", sectionPadding.y)}
      ...
      <div className={cn(maxWidths.xl, sectionPadding.x)}>
        ...
        <div className="text-center mb-12 md:mb-16">
          <SectionTitle className={cn(themeColors.white, "leading-tight mb-6 md:mb-8")}>
            {String(t('title'))} <span className={themeColors.marketing}>{String(t('titleHighlight'))}</span> {String(t('titleEnd') || '')}
          </SectionTitle>
          ...
          <div className="relative mb-12 max-w-md mx-auto">
            ...
            <AnimatePresence>
              {isDropdownOpen && (
                ...
                >
                  {allMarketingCategories.map((category) => (
                    <button
                      key={category.id}
                    ...
                    return idealFor && idealFor !== `categories.${activeCategory}.plans.${plan.id}.idealFor` ? (
                      <div className="px-8 md:px-10 mb-6">
                        <div className={cn(
                          ...
                          <Paragraph size="sm" className="text-white/70 text-center italic">
                            {String(idealFor)}
                          </Paragraph>
                        </div>
                      </div>
